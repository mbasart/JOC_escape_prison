	...
	$("#logIn").click(function(){
		if($("#password").val() != "" && $("#user").val()!=  ""){
			$.ajax({
	            ...
	            type: "POST",
	            dataType: "json",	
	            url: 'http://127.0.0.1:8080/APIGame/user/login',
	            data: JSON.stringify({
					userName: $("#user").val(),
	...
	$("#register").click(function(){
		if($("#password").val() != "" && $("#user").val()!=  ""){
			$.ajax({
	            ...
	            type: "POST",
	            dataType: "json",	
	            url: 'http://127.0.0.1:8080/APIGame/user/register',
	            data: JSON.stringify({
					userName: $("#user").val(),
	              	password: $("#password").val()
				}),
	...
	*/
	...
	if(window.document.title == "Admin User List"){
		...
		$.ajax({
		 	contentType: "application/json;charset=utf-8",
            type: "GET",
            url: 'http://127.0.0.1:8080/APIGame/user/loadUsers',
            success: function(data)
            {
	...
	*/
	...
	if(window.document.title == "Admin User Info"){
		...
		$.ajax({
		 	contentType: "application/json;charset=utf-8",
            type: "GET",
            url: 'http://127.0.0.1:8080/APIGame/game/gameList/'+localStorage.getItem("userInfoName"),
            success: function(data)
            {
            	if(data.length > 0){
            		for (var i = 0; i < data.length; i++) {
	...
	$("#ban").click(function(){
		$.ajax({
            ...
            type: "PATCH",
            dataType: "json",
            url: 'http://127.0.0.1:8080/APIGame/user/banned/',
	...
	$("#admin").click(function(){
		$.ajax({
            ...
            type: "PATCH",
            dataType: "json",
            url: 'http://127.0.0.1:8080/APIGame/user/admin/',
            data: JSON.stringify(localStorage.getItem("userInfoName")),
            success: function(data)
            {
                console.log(data);
            ...
            success: function(data)
            {
                ...
                for (i = 0; i < data.length; i++) {
            		...
            		$.ajax({
					 	contentType: "application/json;charset=utf-8",
			            type: "GET",
			            url: 'http://127.0.0.1:8080/APIGame/game/gameList/'+name,
			            success: function(data)
			            {
			            	var j;
	...
	*/
	...
	if(window.document.title == "My Account"){
		...
		$.ajax({
		 	contentType: "application/json;charset=utf-8",
            type: "GET",
            url: 'http://127.0.0.1:8080/APIGame/game/gameList/'+localStorage.getItem("userName"),
            success: function(data)
            {
            	if(data.length > 0){
            		for (var i = 0; i < data.length; i++) {
